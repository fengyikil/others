...
unsigned int __fifo_len(struct fifo* f)
{
    return (f->in - f->out);
}
...
unsigned int __fifo_get(struct fifo* f,void *buf,unsigned int len)
{
    unsigned int l;
...
unsigned int __fifo_put(struct fifo* f,void *buf,unsigned int len)
{
    unsigned int l;
    unsigned char *buffer = buf;
    l = f->size - f->in + f->out;
...
void block_fifo_put(struct fifo *f,void *buffer,unsigned int len)
{
